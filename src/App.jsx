...
const INVENTORY_CATEGORIES = ["Medications","Supplies","Equipment","Other"];
const INVENTORY_UNITS = ["vials","units","boxes","bottles","syringes","patches","tablets","other"];
...
const SAMPLE_DATA = [
  {
    ...
    consentData: { fullName:"Maria Lopez", dob:"1985-04-12", phone:"713-555-0192", email:"maria@email.com", emergencyContact:"Juan Lopez 713-555-0100", emergencyPhone:"713-555-0100", medicalHistory:"None", currentMeds:"None", allergies:"Penicillin, Sulfa drugs", signature:"Maria Lopez", agreeTreatment:true, agreePhotos:true, agreePolicy:true },
    appointments:[
    ...
    photos:[
      { id:"ph1", date:"2026-05-01", label:"Before", dataUrl:"", weight:"218" },
    ],
    files:[],
  },
];
...
const SAMPLE_INVENTORY = [
  ...
  { id:"i3", name:"Vitamin B12 Injection", category:"Medications", quantity:50, unit:"vials", lowAlert:10, notes:"" },
  { id:"i4", name:"Syringes 1ml", category:"Supplies", quantity:200, unit:"syringes", lowAlert:30, notes:"" },
];

// ── UI Atoms ──────────────────────────────────────────────
function Lbl({ children }) {
  return <div style={{ fontSize:11, fontWeight:700, color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:4 }}>{children}</div>;
}
function Inp({ value, onChange, type="text", placeholder="" }) {
  return <input type={type} value={value} onChange={onChange} placeholder={placeholder}
    style={{ width:"100%", padding:"9px 11px", border:`1.5px solid ${C.border}`, borderRadius:8, fontSize:13, color:C.text, background:"#fff", boxSizing:"border-box", fontFamily:"inherit", outline:"none" }} />;
}
function Txa({ value, onChange, placeholder="", rows=3 }) {
  return <textarea value={value} onChange={onChange} placeholder={placeholder} rows={rows}
    style={{ width:"100%", padding:"9px 11px", border:`1.5px solid ${C.border}`, borderRadius:8, fontSize:13, color:C.text, background:"#fff", boxSizing:"border-box", fontFamily:"inherit", outline:"none", resize:"vertical" }} />;
}
...
function Pill({ children, color="rose" }) {
  const map = {
    rose:{bg:"#fce8f3",fg:"#b0608c"}, mint:{bg:"#e0f5ef",fg:"#4a9e89"},
    amber:{bg:"#fef5dc",fg:"#a07830"}, red:{bg:"#fde8e8",fg:"#b05050"},
    gray:{bg:"#f5eef8",fg:"#7a5a8a"}, blue:{bg:"#e8f0fe",fg:"#3b5bdb"},
    purple:{bg:"#f3e8ff",fg:"#7c3aed"},
  };
  const s = map[color] || map.rose;
  return <span style={{ display:"inline-block", padding:"2px 10px", borderRadius:999, fontSize:11, fontWeight:700, background:s.bg, color:s.fg }}>{children}</span>;
}
function Btn({ children, onClick, variant="primary", small=false }) {
  const styles = {
    ...
    blue:{ background:"#e8f0fe", color:"#3b5bdb", border:"none" },
  };
  return <button onClick={onClick}
    style={{ ...styles[variant], borderRadius:9, padding:small?"5px 12px":"9px 18px", fontWeight:700, fontSize:small?12:13, cursor:"pointer", fontFamily:"inherit" }}>
    {children}
  </button>;
}
function SectionHead({ children }) {
  return <div style={{ fontWeight:800, fontSize:13, color:C.accent, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:12, paddingBottom:6, borderBottom:`1px solid ${C.border}` }}>{children}</div>;
}
...
function F({ label, full=false, children }) {
  return <div style={{ gridColumn:full?"1/-1":"auto" }}><Lbl>{label}</Lbl>{children}</div>;
}
function Empty({ icon, msg }) {
  return <div style={{ textAlign:"center", padding:"40px 0", color:C.muted }}>
...
function WeightBar({ start, current, goal }) {
  ...
  return <div style={{ marginTop:12 }}>
    <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:C.muted, marginBottom:4 }}>
      <span>Progress to goal</span><span style={{ fontWeight:700, color:C.accent }}>{pct}%</span>
    </div>
...
const lastVisitDate = (visits) => { if(!visits||!visits.length) return null; return [...visits].sort((a,b)=>b.date.localeCompare(a.date))[0].date; };
const nextApptOf = (appts) => { if(!appts) return null; const f=appts.filter(a=>a.date>=TODAY&&a.status!=="Cancelled"&&a.status!=="No Show"); if(!f.length) return null; return [...f].sort((a,b)=>a.date.localeCompare(b.date)||APPT_TIMES.indexOf(a.time)-APPT_TIMES.indexOf(b.time))[0]; };
...
const emptyPatient = () => ({ id:uid(), firstName:"", lastName:"", dob:"", phone:"", email:"", gender:"", status:"Active", insurance:"", emergencyContact:"", allergies:"", startWeight:"", currentWeight:"", goalWeight:"", heightIn:"", medications:[], labResults:"", imaging:"", consentSigned:false, consentDate:"", consentData:null, appointments:[], visits:[], billing:[], photos:[], files:[] });
...
const emptyInventory = () => ({ id:uid(), name:"", category:"", quantity:"", unit:"", lowAlert:"", notes:"" });
const emptyConsent = () => ({ fullName:"", dob:"", phone:"", email:"", emergencyContact:"", emergencyPhone:"", medicalHistory:"", currentMeds:"", allergies:"", signature:"", agreeTreatment:false, agreePhotos:false, agreePolicy:false });
      ...
      <div style={{ background:"#fff", borderRadius:20, padding:28, width:"100%", maxWidth:480, boxShadow:"0 8px 32px rgba(194,24,91,0.2)", maxHeight:"90vh", overflowY:"auto" }}>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:16 }}>
          <thead>
            <tr style={{ background:C.bg }}>
              {["Date","Description","Charged","Paid"].map(h => (
                <th key={h} style={{ padding:"8px", textAlign:"left", fontSize:11, color:C.muted, fontWeight:700, textTransform:"uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {patient.billing.map(b => (
              <tr key={b.id} style={{ borderBottom:`1px solid ${C.border}` }}>
                <td style={{ padding:"8px", fontSize:12, color:C.text }}>{fmt(b.date)}</td>
                <td style={{ padding:"8px", fontSize:12, color:C.text }}>{b.description}</td>
                <td style={{ padding:"8px", fontSize:12, color:C.text }}>{currency(b.amount)}</td>
                <td style={{ padding:"8px", fontSize:12, color:"#4a9e89" }}>{currency(b.paid)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <div style={{ background:C.bg, borderRadius:10, padding:"12px 16px", marginBottom:20 }}>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, marginBottom:4 }}>
            <span style={{ color:C.muted }}>Total Charged</span><span style={{ fontWeight:700 }}>{currency(t.charged)}</span>
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, marginBottom:4 }}>
            <span style={{ color:C.muted }}>Total Paid</span><span style={{ fontWeight:700, color:"#4a9e89" }}>{currency(t.paid)}</span>
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:15, borderTop:`1px solid ${C.border}`, paddingTop:8, marginTop:4 }}>
            <span style={{ fontWeight:800, color:C.text }}>Balance Due</span>
            <span style={{ fontWeight:800, color:t.owed>0?"#a07830":"#4a9e89" }}>{currency(t.owed)}</span>
          </div>
        </div>
...
function ConsentModal({ patient, onSave, onClose }) {
  ...
  const submit = () => {
    ...
    if (!form.agreeTreatment || !form.agreePolicy) return alert("Please agree to all required terms.");
    onSave(form);
  };

  return (
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(61,36,56,0.5)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20, overflowY:"auto" }}>
      <div style={{ background:"#fff", borderRadius:20, padding:24, width:"100%", maxWidth:520, boxShadow:"0 8px 32px rgba(194,24,91,0.2)", maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ textAlign:"center", marginBottom:20 }}>
          <div style={{ fontSize:20, fontWeight:800, color:C.accent }}>🌸 Sulem Ageless Slimming</div>
          <div style={{ fontSize:14, fontWeight:700, color:C.text, marginTop:4 }}>Patient Consent Form</div>
        </div>
        ...
        <Row2>
          <F label="Medical History / Conditions" full><Txa value={form.medicalHistory} onChange={f("medicalHistory")} placeholder="List any medical conditions..." /></F>
          <F label="Current Medications" full><Txa value={form.currentMeds} onChange={f("currentMeds")} placeholder="List all current medications..." /></F>
          <F label="Allergies" full><Inp value={form.allergies} onChange={f("allergies")} placeholder="List all allergies or write None" /></F>
        </Row2>

        <SectionHead>Consent & Agreement</SectionHead>
        <div style={{ background:C.bg, borderRadius:10, padding:14, marginBottom:12, fontSize:12, color:C.text, lineHeight:1.6 }}>
          I understand that weight loss treatments including medications and injections carry risks. I consent to treatment by Sulem Ageless Slimming staff and agree to follow all prescribed protocols. I understand results may vary and no specific outcome is guaranteed.
        </div>
        {[
          ["agreeTreatment", "I consent to weight loss treatment and understand the risks *"],
          ["agreePhotos", "I consent to before/after photos being taken for my chart (optional)"],
          ["agreePolicy", "I agree to the clinic's cancellation and payment policy *"],
        ].map(([key, label]) => (
          <label key={key} style={{ display:"flex", alignItems:"flex-start", gap:10, marginBottom:10, cursor:"pointer" }}>
            <input type="checkbox" checked={form[key]} onChange={fc(key)} style={{ marginTop:2, accentColor:C.accent }} />
            <span style={{ fontSize:13, color:C.text }}>{label}</span>
          </label>
        ))}

        <div style={{ marginTop:16 }}>
          <Lbl>Signature (type full name) *</Lbl>
          <Inp value={form.signature} onChange={f("signature")} placeholder="Type your full name as signature" />
          <div style={{ fontSize:11, color:C.muted, marginTop:4 }}>Date: {new Date().toLocaleDateString()}</div>
        </div>
...
export default function App() {
  const [patients, setPatients] = useState(() => {
    try { const s=localStorage.getItem("sulem_clinic_patients"); return s?JSON.parse(s):SAMPLE_DATA; } catch { return SAMPLE_DATA; }
  });
  const [inventory, setInventory] = useState(() => {
    try { const s=localStorage.getItem("sulem_clinic_inventory"); return s?JSON.parse(s):SAMPLE_INVENTORY; } catch { return SAMPLE_INVENTORY; }
  });

  useEffect(() => { try { localStorage.setItem("sulem_clinic_patients", JSON.stringify(patients)); } catch {} }, [patients]);
  useEffect(() => { try { localStorage.setItem("sulem_clinic_inventory", JSON.stringify(inventory)); } catch {} }, [inventory]);
  ...
  const handlePhoto = (e, label, weight) => {
    const f = e.target.files[0];
    if (!f) return;
    const r = new FileReader();
    r.onload = (ev) => {
      const obj = {id:uid(), date:TODAY, label:label||"Photo", dataUrl:ev.target.result, weight:weight||""};
      upActive(p => ({...p, photos:[...(p.photos||[]), obj]}));
    };
    r.readAsDataURL(f);
    e.target.value = "";
  };
  ...
  const saveConsent = (data) => {
    upActive(p => ({...p, consentSigned:true, consentDate:TODAY, consentData:data}));
      ...
      <div style={{ background:hGrad, padding:"18px 20px 16px", color:C.text, boxShadow:"0 2px 16px rgba(220,170,210,0.3)" }}>
        <div style={{ maxWidth:780, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            {page==="inventory" && <Btn onClick={() => { setInvForm(emptyInventory()); setEditInvId(null); }}>+ Add Item</Btn>}
        ...
        <div style={{ maxWidth:780, margin:"0 auto", display:"flex" }}>
          {[["today","📅 Today"],["roster","👥 Roster"],["inventory","📦 Inventory"]].map(([v,label]) => (
            <button key={v} onClick={() => setPage(v)}
              style={{ flex:1, padding:"12px 8px", border:"none", fontWeight:700, fontSize:12, cursor:"pointer", fontFamily:"inherit", background:"transparent",
                color:page===v?C.accent:C.muted, borderBottom:page===v?`3px solid ${C.accent}`:"3px solid transparent", position:"relative" }}>
              {label}
              {v==="inventory" && lowItems.length>0 && (
                <span style={{ position:"absolute", top:8, right:"20%", background:C.red, color:"#b05050", borderRadius:99, fontSize:9, fontWeight:800, padding:"1px 5px" }}>{lowItems.length}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth:780, margin:"0 auto", padding:"0 16px" }}>
          ...
          <div style={{ marginTop:20 }}>
            ...
            {lowItems.length>0 && (
              <div style={{ background:"#fff5f5", border:"1.5px solid #fcd0d0", borderRadius:12, padding:"12px 16px", marginBottom:16 }}>
                ...
                {lowItems.map(i => (
                  <div key={i.id} style={{ fontSize:12, color:"#b05050" }}>• {i.name}: only {i.quantity} {i.unit} left</div>
                ))}
              </div>
            )}
        ...
        {page==="roster" && (
          <div style={{ marginTop:20 }}>
            <input placeholder="Search by name, phone, or email..." value={search} onChange={e=>setSearch(e.target.value)}
              style={{ width:"100%", padding:"10px 14px", border:`1.5px solid ${C.border}`, borderRadius:10, fontSize:13, background:"#fff", boxSizing:"border-box", fontFamily:"inherit", outline:"none", marginBottom:10 }} />
            <div style={{ display:"flex", gap:6, marginBottom:16, flexWrap:"wrap" }}>
              {["All","Active","On Hold","Inactive"].map(s => (
                <button key={s} onClick={() => setStFilter(s)}
                  style={{ padding:"4px 14px", borderRadius:99, border:"none", cursor:"pointer", fontWeight:700, fontSize:12, fontFamily:"inherit", background:stFilter===s?C.accent:"#f5e4ef", color:stFilter===s?"#fff":C.muted }}>
                  {s}
                </button>
              ))}
            </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => openChart(p)}
                  style={{ background:C.card, borderRadius:14, padding:"16px", marginBottom:10, boxShadow:"0 1px 6px rgba(194,24,91,0.07)", cursor:"pointer", borderLeft:`4px solid ${p.status==="Inactive"?C.muted:p.status==="On Hold"?C.amber:C.hero1}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div style={{ flex:1 }}>
        ...
        {page==="inventory" && (
          <div style={{ marginTop:20 }}>
            {lowItems.length>0 && (
              <div style={{ background:"#fff5f5", border:"1.5px solid #fcd0d0", borderRadius:12, padding:"12px 16px", marginBottom:16 }}>
                <div style={{ fontSize:12, fontWeight:700, color:"#b05050", marginBottom:6 }}>⚠️ Low Stock — Reorder Needed</div>
                {lowItems.map(i => <div key={i.id} style={{ fontSize:12, color:"#b05050" }}>• {i.name}: {i.quantity} {i.unit} remaining</div>)}
              </div>
            )}
            ...
            {invForm && (
              <div style={{ background:C.card, borderRadius:16, padding:20, boxShadow:"0 1px 8px rgba(194,24,91,0.08)", marginBottom:16 }}>
                ...
                <Row2>
                  ...
                  <F label="Category"><Sel value={invForm.category} onChange={e=>setInvForm(f=>({...f,category:e.target.value}))} options={INVENTORY_CATEGORIES} /></F>
                  <F label="Quantity *"><Inp type="number" value={invForm.quantity} onChange={e=>setInvForm(f=>({...f,quantity:e.target.value}))} /></F>
                  <F label="Unit"><Sel value={invForm.unit} onChange={e=>setInvForm(f=>({...f,unit:e.target.value}))} options={INVENTORY_UNITS} /></F>
            ...
            {["Medications","Supplies","Equipment","Other"].map(cat => {
              ...
              <div style={{ background:C.card, borderRadius:16, padding:18, boxShadow:"0 1px 8px rgba(194,24,91,0.08)", marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                  <div style={{ flex:1 }}>
                    ...
                    {active.allergies && (
                      <div style={{ marginTop:8, background:"#fff0f0", border:"1px solid #fcd0d0", borderRadius:8, padding:"6px 10px", fontSize:12, color:"#b05050", fontWeight:700 }}>
                        ⚠️ ALLERGIES: {active.allergies}
                      </div>
                    )}
                    <div style={{ marginTop:8, display:"flex", gap:6, flexWrap:"wrap" }}>
                      {active.status && <Pill color={statusColor(active.status)}>{active.status}</Pill>}
                      {active.consentSigned ? <Pill color="mint">✓ Consent Signed {fmt(active.consentDate)}</Pill> : <Pill color="red">⚠️ No Consent</Pill>}
                      {lost&&Number(lost)>0 && <Pill color="mint">↓ {lost} lbs lost</Pill>}
              ...
              <div style={{ display:"flex", gap:4, marginBottom:16, background:"#f3e0e9", borderRadius:12, padding:4 }}>
                {[["appts","📅"],["overview","📋"],["visits","🩺"],["photos","📸"],["billing","💳"],["files","📎"]].map(([v,label]) => (
                  <button key={v} onClick={() => setTab(v)}
                    style={{ flex:1, padding:"8px 4px", border:"none", borderRadius:9, fontWeight:700, fontSize:11, cursor:"pointer", fontFamily:"inherit",
                      background:tab===v?"#fff":"transparent", color:tab===v?C.accent:C.muted,
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                  ...
                  <div style={{ gridColumn:"1/-1", background:C.card, borderRadius:12, padding:"14px 16px", boxShadow:"0 1px 4px rgba(194,24,91,0.07)" }}>
                    ...
                    {(active.medications||[]).filter(Boolean).length===0
                      ...
                      : (active.medications||[]).filter(Boolean).map((m,i) => <div key={i} style={{ fontSize:13, color:C.text, padding:"4px 0", borderBottom:`1px solid ${C.border}` }}>{m}</div>)}
                  </div>
                  {active.labResults && (
                    <div style={{ gridColumn:"1/-1", background:C.card, borderRadius:12, padding:"14px 16px", boxShadow:"0 1px 4px rgba(194,24,91,0.07)" }}>
              ...
              {tab==="photos" && (
                <div>
                  <input type="file" ref={photoRef} accept="image/*" style={{ display:"none" }}
                    onChange={e => {
                      const label = prompt("Label this photo (e.g. Before, After, Week 4):");
                      const weight = prompt("Weight at time of photo (lbs):");
                      handlePhoto(e, label, weight);
                    }} />
                  <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:12 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    {(active.photos||[]).map(ph => (
                      <div key={ph.id} style={{ background:C.card, borderRadius:12, overflow:"hidden", boxShadow:"0 1px 4px rgba(194,24,91,0.07)" }}>
                        {ph.dataUrl
                          ? <img src={ph.dataUrl} alt={ph.label} style={{ width:"100%", height:180, objectFit:"cover" }} />
                          : <div style={{ height:180, background:C.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:40 }}>📷</div>
                        }
                        <div style={{ padding:"10px 12px" }}>
                          ...
                          <button onClick={() => upActive(p => ({...p, photos:(p.photos||[]).filter(x=>x.id!==ph.id)}))}
                            style={{ marginTop:6, fontSize:11, fontWeight:700, color:"#b05050", background:"#fee2e2", border:"none", padding:"3px 10px", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>
                            Remove
                          </button>
                        </div>
                      </div>
              ...
              {tab==="files" && (
                <div>
                  ...
                  <div onClick={() => fileRef.current.click()}
                    style={{ border:`2px dashed ${C.border}`, borderRadius:14, padding:32, textAlign:"center", cursor:"pointer", marginBottom:16, background:"#fff9fc" }}>
                      ...
                      <div key={fi.id} style={{ background:C.card, borderRadius:12, padding:12, boxShadow:"0 1px 4px rgba(194,24,91,0.07)" }}>
                        ...
                        <div style={{ display:"flex", gap:6, marginTop:8 }}>
                          <a href={fi.dataUrl} download={fi.name}
                            style={{ fontSize:11, fontWeight:700, color:C.accent, textDecoration:"none", background:"#fce4ec", padding:"3px 10px", borderRadius:6 }}>Download</a>
                          <button onClick={() => upActive(p => ({...p, files:p.files.filter(x=>x.id!==fi.id)}))}
                            style={{ fontSize:11, fontWeight:700, color:"#b05050", background:"#fee2e2", border:"none", padding:"3px 10px", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>Remove</button>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(61,36,56,0.4)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:C.card, borderRadius:20, padding:24, width:"100%", maxWidth:460, boxShadow:"0 8px 32px rgba(194,24,91,0.2)" }}>
            ...
            <div style={{ marginBottom:12 }}>
              <Lbl>Patient *</Lbl>
              <select value={quickPtId} onChange={e=>setQuickPtId(e.target.value)}
                style={{ width:"100%", padding:"9px 11px", border:`1.5px solid ${C.border}`, borderRadius:8, fontSize:13, color:C.text, background:"#fff", boxSizing:"border-box", fontFamily:"inherit", outline:"none" }}>
